...
#nice .imageflow-img {
  ...
  `},oe=[z,re,se];async function le(){return oe}async function Q(a){var n;const e=document.createElement("div");e.innerHTML=a;const t=e.querySelectorAll("img");for(const i of Array.from(t)){let r=i.src||i.getAttribute("src")||"";if(r.startsWith("attachment:")){const o=(n=window.location.href.match(/([a-f0-9]{32})/))==null?void 0:n[1];o&&(r=`https://www.notion.so/image/attachment:${r.replace("attachment:","")}?table=block&id=${o}`,i.setAttribute("src",r))}if(r.includes("notion.so")||r.includes("notionusercontent.com")||r.includes("file.notion.so"))try{const s=await fetch(r);if(!s.ok)throw new Error("Failed to download image");const o=await s.blob(),l=await new Promise((u,p)=>{const f=new FileReader;f.onload=()=>u(f.result),f.onerror=p,f.readAsDataURL(o)});i.src=l}catch(s){console.warn("Failed to process image:",s)}}return e.innerHTML}function F(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}let v=F();function G(a){v=a}const J=/[&<>"']/,ae=new RegExp(J.source,"g"),V=/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,ce=new RegExp(V.source,"g"),pe={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},X=a=>pe[a];function k(a,e){if(e){if(J.test(a))return a.replace(ae,X)}else if(V.test(a))return a.replace(ce,X);return a}const he=/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig;function ue(a){return a.replace(he,(e,t)=>(t=t.toLowerCase(),t==="colon"?":":t.charAt(0)==="#"?t.charAt(1)==="x"?String.fromCharCode(parseInt(t.substring(2),16)):String.fromCharCode(+t.substring(1)):""))}const de=/(^|[^\[])\^/g;function b(a,e){a=typeof a=="string"?a:a.source,e=e||"";const t={replace:(n,i)=>(i=typeof i=="object"&&"source"in i?i.source:i,i=i.replace(de,"$1"),a=a.replace(n,i),t),getRegex:()=>new RegExp(a,e)};return t}function W(a){try{a=encodeURI(a).replace(/%25/g,"%")}catch{return null}return a}const E={exec:()=>null};function K(a,e){const t=a.replace(/\|/g,(r,s,o)=>{let l=!1,u=s;for(;--u>=0&&o[u]==="\\";)l=!l;return l?"|":" |"}),n=t.split(/ \|/);let i=0;if(n[0].trim()||n.shift(),n.length>0&&!n[n.length-1].trim()&&n.pop(),e)if(n.length>e)n.splice(e);else for(;n.length<e;)n.push("");for(;i<n.length;i++)n[i]=n[i].trim().replace(/\\\|/g,"|");return n}function q(a,e,t){const n=a.length;if(n===0)return"";let i=0;for(;i<n;){const r=a.charAt(n-i-1);if(r===e&&!t)i++;else if(r!==e&&t)i++;else break}return a.slice(0,n-i)}function fe(a,e){if(a.indexOf(e[1])===-1)return-1;let t=0;for(let n=0;n<a.length;n++)if(a[n]==="\\")n++;else if(a[n]===e[0])t++;else if(a[n]===e[1]&&(t--,t<0))return n;return-1}function Y(a,e,t,n){const i=e.href,r=e.title?k(e.title):null,s=a[1].replace(/\\([\[\]])/g,"$1");if(a[0].charAt(0)!=="!"){n.state.inLink=!0;const o={type:"link",raw:t,href:i,title:r,text:s,tokens:n.inlineTokens(s)};return n.state.inLink=!1,o}return{type:"image",raw:t,href:i,title:r,text:k(s)}}function ge(a,e){const t=a.match(/^(\s+)(?:```)/);if(t===null)return e;const n=t[1];return e.split(`
...
`,e=e.substring(d.length+1),p=!0),!p){const P=new RegExp(`^ {0,${Math.min(3,m-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),H=new RegExp(`^ {0,${Math.min(3,m-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),O=new RegExp(`^ {0,${Math.min(3,m-1)}}(?:\`\`\`|~~~)`),U=new RegExp(`^ {0,${Math.min(3,m-1)}}#`);for(;e;){const Z=e.split(`
`,1)[0];if(d=Z,this.options.pedantic&&(d=d.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),O.test(d)||U.test(d)||P.test(d)||H.test(e))break;if(d.search(/[^ ]/)>=m||!d.trim())l+=`
`+d.slice(m);else{if(T||f.search(/[^ ]/)>=4||O.test(f)||U.test(f)||H.test(f))break;l+=`
`+d}!T&&!d.trim()&&(T=!0),o+=Z+`
...
Please report this to https://github.com/markedjs/marked.`,e){const i="<p>An error occurred:</p><pre>"+k(n.message+"",!0)+"</pre>";return t?Promise.resolve(i):i}if(t)return Promise.reject(n);throw n}};const S=new be;function g(a,e){return S.parse(a,e)}g.options=g.setOptions=function(a){return S.setOptions(a),g.defaults=S.defaults,G(g.defaults),g};g.getDefaults=F;g.defaults=v;g.use=function(...a){return S.use(...a),g.defaults=S.defaults,G(g.defaults),g};g.walkTokens=function(a,e){return S.walkTokens(a,e)};g.parseInline=S.parseInline;g.Parser=$;g.parser=$.parse;g.Renderer=B;g.TextRenderer=N;g.Lexer=y;g.lexer=y.lex;g.Tokenizer=L;g.Hooks=R;g.parse=g;g.options;g.setOptions;g.use;g.walkTokens;g.parseInline;$.parse;y.lex;const A=new g.Renderer;A.heading=function(a,e){return e===1?`
    ...
    `:`<h${e}>${a}</h${e}>`};A.listitem=function(a){return`<li><section>${a}</section></li>`};A.blockquote=function(a){return`<blockquote class="multiquote-1">${a}</blockquote>`};A.image=function(a,e,t){const n=e?` title="${e}"`:"";return`<img src="${a}" alt="${t}"${n}>`};function me(a,e){return g.setOptions({renderer:A}),{html:g(a),images:e}}function xe(a,e=z){return`<div id="nice">${a}</div><style>${e.styles}</style>`}class ke{constructor(){x(this,"sidebar",null);x(this,"button",null);x(this,"availableThemes",[z]);this.init()}async init(){this.createFloatingButton(),this.attachButtonEvent(),this.loadThemes().then(()=>{console.log("Themes loaded:",this.availableThemes.map(e=>e.name))})}async loadThemes(){try{this.availableThemes=await le()}catch(e){console.warn("Failed to load themes:",e),this.availableThemes=[z]}}attachButtonEvent(){var e;(e=this.button)==null||e.addEventListener("click",()=>this.openSidebar())}openSidebar(){if(this.sidebar){this.closeSidebar();return}this.createSidebar(),document.body.appendChild(this.sidebar),setTimeout(()=>{this.sidebar&&(this.sidebar.style.transform="translateX(0)")},10),this.attachSidebarEvents()}createFloatingButton(){this.button=document.createElement("div"),this.button.id="notion2wechat-button",this.button.innerHTML=`
